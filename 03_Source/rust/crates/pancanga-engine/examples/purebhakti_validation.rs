...
const DEFAULT_INPUT: &str = "../../04_Tests/PureBhakti/purebhakti-fixtures.csv";
const DEFAULT_OUTPUT: &str = "../../04_Tests/PureBhakti/purebhakti-validation.csv";
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if !config.input.is_file() {
        fs::write(
            &config.output,
            pending_csv("PureBhakti fixtures are not available"),
        )?;
        println!("External Oracle Pending: PureBhakti fixtures not found.");
    ...
    if fixtures.is_empty() {
        fs::write(
            &config.output,
            pending_csv("PureBhakti fixture CSV has no rows"),
        )?;
        println!("External Oracle Pending: PureBhakti fixture CSV has no rows.");
        return Ok(());
    }
    ...
    println!("PureBhakti validation rows: {}", fixtures.len());
...
fn observance_match(
    ...
    if current.disposition == EkadasiObservanceDisposition::ObserveOnEkadasi {
        return (
            ...
            Some(current.date),
            "PASS".to_string(),
            "PureBhakti date matches current-day valid Ekadasi observance".to_string(),
        );
    }
    ...
    if previous.disposition == EkadasiObservanceDisposition::ObserveOnDvadasi {
        return (
            ...
            Some(fixture_date),
            "PASS".to_string(),
            "PureBhakti date matches Dvadasi observance displaced from previous-day Viddha candidate"
                .to_string(),
        );
    }
...
fn validate_parana(
    ...
    let engine_start = local_hhmm(window.starts_at, BUENOS_AIRES_UTC_OFFSET_HOURS);
    let engine_end = local_hhmm(window.ends_at, BUENOS_AIRES_UTC_OFFSET_HOURS);
    ...
    let notes = if status == "PASS" {
        "PureBhakti Parana window matches engine-calculated HBV-EK-005 window".to_string()
    } else {
        format!(
            "PureBhakti Parana {}-{} differs from engine {}-{}; classify before changing code",
...
fn mahadvadasi_note(fixture: &Fixture) -> String {
    if fixture.mahadvadasi_type == "None" {
        ...
        format!(
            "PureBhakti marks Mahadvadasi type {}; automatic Mahadvadasi fact derivation is not part of this validator",
...
fn pending_csv(notes: &str) -> String {
    format!(
        "{OUTPUT_HEADERS}PB-0000,,,,,,,,,,,,,,EXTERNAL_ORACLE_PENDING,EXTERNAL_ORACLE_PENDING,{notes}\n"
    )
}
